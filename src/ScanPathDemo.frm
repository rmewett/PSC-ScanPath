...
Begin VB.Form frmMain 
   ...
   Caption         =   "ScanPath 2.0.1 - Simple Demonstration of how to use in your own apps...."
   ClientHeight    =   8700
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   11355
   ...
   Begin VB.CheckBox chkExtended 
      Caption         =   "Use Extended Mode Scan"
      Height          =   195
      Left            =   1530
      TabIndex        =   11
      Top             =   2850
...
Private Sub cmdScan_Click()
    ...
    If cmdScan.Caption = "Scan" Then
        ...
        With SP
            ...
            .StartScan txtPath, chkSubFolders.Value, chkSort.Value And chkSort.Enabled, chkPatternMatching, chkExtended.Value
        End With
...
Private Sub Form_Load()
    ...
    With ListView1
        ...
        .ColumnHeaders.Add , "ATTRIB", "Attrib", 0, lvwColumnLeft
...
Private Sub GetFileStructure()
    ...
    'This can be used to access the API file structure of the current dir/file that
    'had been retrieved.
...
Private Sub SP_DirMatch(Directory As String, Path As String)
    '####################################################################################################
    'This Event fires for each Folder found  - Used when not running in Extended Detail Mode
    '####################################################################################################
End Sub

Private Sub SP_DirMatchEx(Directory As String, Path As String, Attributes As Long, LastWriteTime As Date, CreationTime As Date, LastAccessTime As Date)
    '####################################################################################################
    'This Event fires for each Folder found  - Used when running in Extended Detail Mode
    '###################################################################################################
End Sub
...
Private Sub SP_FileMatch(Filename As String, Path As String)
    ...
    '####################################################################################################
    'This Event fires for each File found  - Used when not running in Extended Detail Mode
    '####################################################################################################
...
Private Sub SP_FileMatchEx(Filename As String, Path As String, FileSize As Long, Attributes As Long, LastWriteTime As Date, CreationTime As Date, LastAccessTime As Date)
    Dim LV As ListItem
    Dim sAttrib As String
    
    '####################################################################################################
    'This Event fires for each File found  - Used when running in Extended Detail Mode
    
    'The difference between this and the FileMatch Event is that it returns'
    'FileSize, Attribute & Dates as well as the FileName and Path. This makes
    'is slightly slower when using Sorted scans because it must rescan the File to get
    'the file details - hence the faster Event is available if you do not need the
    'details.
    '####################################################################################################
    ...
    With ListView1
        Set LV = .ListItems.Add(, , Filename)
        LV.SubItems(1) = Path
        LV.SubItems(2) = FileSize
        LV.SubItems(3) = LastAccessTime
        
        sAttrib = Space$(4)
        If Attributes And FILE_ATTRIBUTE_ARCHIVE Then
            Mid$(sAttrib, 1, 1) = "a"
        End If
        If Attributes And FILE_ATTRIBUTE_HIDDEN Then
            Mid$(sAttrib, 2, 1) = "h"
        End If
        If Attributes And FILE_ATTRIBUTE_READONLY Then
            Mid$(sAttrib, 3, 1) = "r"
        End If
        If Attributes And FILE_ATTRIBUTE_SYSTEM Then
            Mid$(sAttrib, 4, 1) = "s"
        End If
        
        LV.SubItems(4) = Trim$(sAttrib)
